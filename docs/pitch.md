...
**"Is the demo canned?"**
No. Every story is a live streaming call to claude-sonnet-5; there are no pre-generated stories in the repo. Two things are cached: narration audio, hashed by text and voice so a rerun replays from disk, and one saved source corpus — real Wikipedia data fetched earlier — as insurance in case Wikipedia itself is unreachable on stage. The weave still runs live even from that. Then: "Give me any topic and we'll run it right now." Take the topic.
